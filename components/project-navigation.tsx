...
export function ProjectNavigation({ currentSlug }: ProjectNavigationProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-6 p-8 bg-muted-gray/30 rounded-2xl border border-off-white/10">
      {/* Previous Project */}
      <Link
        href={`/work/${prevProject.slug}`}
        className="group flex items-center gap-4 text-left hover:text-neon-lime focus-ring rounded-xl p-4 -m-4"
      >
        <div className="w-10 h-10 bg-off-white/10 group-hover:bg-neon-lime/20 rounded-full flex items-center justify-center">
          <ChevronLeft className="w-5 h-5" />
        </div>
        <div>
          <p className="text-off-white/70 text-sm mb-1">Previous Project</p>
          <h3 className="font-display font-semibold text-off-white group-hover:text-neon-lime">
            {prevProject.title}
          </h3>
      ...
      <Link
        ...
        className="group flex items-center gap-4 text-right hover:text-neon-lime focus-ring rounded-xl p-4 -m-4"
      >
        ...
        <div className="w-10 h-10 bg-off-white/10 group-hover:bg-neon-lime/20 rounded-full flex items-center justify-center order-1">
